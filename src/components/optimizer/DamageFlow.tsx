          ...
          {result.baseDebug && (
            <>
              ...
              {(result.baseDebug.atkRatio ?? 0) > 0 && (
                <FlowRow label="倍率" value={`×${fmt(result.baseDebug.atkRatio)}`} sub={result.baseDebug.atkRatio > 0 ? `${(result.baseDebug.atkRatio * 100).toFixed(4)}%` : ''} />
              )}
              {(result.baseDebug.hpRatio ?? 0) > 0 && (
                <FlowRow label="生命倍率" value={`×${fmt(result.baseDebug.hpRatio)}`} />
              )}
              {(result.baseDebug.defRatio ?? 0) > 0 && (
                <FlowRow label="防御倍率" value={`×${fmt(result.baseDebug.defRatio)}`} />
              )}
              {(result.baseDebug.emRatio ?? 0) > 0 && (
                <FlowRow label="精通倍率" value={`×${fmt(result.baseDebug.emRatio)}`} />
              )}
              {result.baseDebug.baseDmgMultiplier && result.baseDebug.baseDmgMultiplier !== 1 && (
                <FlowRow label="倍率提升" value={`×${fmt(result.baseDebug.baseDmgMultiplier)}`} />
              )}
              {(() => {
                const parts: string[] = [];
